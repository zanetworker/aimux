...
- **Provider badge** (Claude, Codex) with color coding
...
- **Provider filters**: Claude, Codex
- **Location filter**: Local, Remote — show only local agents or only remote sandboxes
...
The web dashboard has full support for [OpenShell remote sandboxes](/guides/remote-sandboxes/). Remote agents appear alongside local agents in the card grid.

### Launching

Click **+ Launch**, set **Runtime: remote**, choose a directory, and click **Launch Agent**. The dashboard navigates directly to the new sandbox's session terminal and starts Claude automatically.

### Card appearance

Remote sandbox cards show:
- **LOC**: `remote` badge in the location column
- **Name**: your project directory (e.g., `research`) — not the raw sandbox ID
- **Status**: Active (green) while ready; **ERROR / Deleting** immediately after Kill

### Session terminal

The **Session** tab connects to a live PTY inside the sandbox. Claude starts with `--session-id <uuid>` on first connect and `--resume <uuid>` on reconnect, keeping the same conversation thread.

### Kill

Click **Kill** on the card — no confirmation dialog. The card switches to **Deleting** state immediately and disappears once the sandbox is fully removed.

### Traces

The **Trace** tab shows turn-by-turn conversation with assistant replies read from the sandbox's session file. Replies refresh every 5 seconds.
