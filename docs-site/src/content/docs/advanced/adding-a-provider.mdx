...
Copy `internal/provider/claude.go`, rename it, and adjust. This gets you a compilable stub that shows up in the launcher.
...
**2. Register in `internal/frontend/tui/app.go` -- add to the `allProviders` slice in `NewApp()`:**
...
- Return `false` if the agent has its own TUI framework that conflicts with Bubble Tea's alternate screen. Codex returns false.
...
### 1. `internal/frontend/tui/app.go` -- `NewApp()`
...
### Step 2: Register in `internal/frontend/tui/app.go`
