...
```
Agent (what to run)
  Primary:  ~/.aimux/agents/<name>.agent.yaml  (one file per agent)
  Inline:   ~/.aimux/config.yaml agents: []    (optional, auto-synced)
  Project:  .aimux/agents/<name>.agent.yaml     (project-local override)
  ...
  Sync: per-file agents are authoritative. The inline agents[] list
        in config.yaml is regenerated on startup from the per-file
        agents. Edits to config.yaml agents[] are accepted but
        written back to per-file on next sync.
...
### Per-agent files (primary)

```
~/.aimux/agents/
  reviewer.agent.yaml
  quick-fix.agent.yaml
  background-worker.agent.yaml
```

```yaml
# ~/.aimux/agents/reviewer.agent.yaml
name: reviewer
harness: claude-sdk
model: opus
prompt: |
...
```yaml
# ~/.aimux/agents/quick-fix.agent.yaml
name: quick-fix
harness: claude-native
model: sonnet
```

Project-local agents override globals by name:

```
my-project/.aimux/agents/
  reviewer.agent.yaml          # overrides global reviewer
  project-helper.agent.yaml   # project-only agent
```

### config.yaml (inline agents + environments)

The `agents:` section is optional and auto-synced from the per-file agents.
Editing either location works; per-file is authoritative on conflict.
