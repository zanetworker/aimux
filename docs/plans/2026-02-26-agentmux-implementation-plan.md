...
### Task 7: k9s-Style Header View

**Files:**
- Create: `internal/tui/views/header.go`
- Modify: `internal/tui/app.go` — use new header
- Modify: `internal/tui/styles.go` — add k9s color palette

**Step 1: Update styles.go with k9s palette**
...
// HeaderView renders the k9s-style header with info boxes and logo.
...
```bash
git add -A
git commit -m "feat: k9s-style header with info boxes and ASCII logo"
```

---

### Task 8: Rename Instances View → Agents View (k9s Table Style)

**Files:**
- Rename: `internal/tui/views/instances.go` → `internal/tui/views/agents.go`
- Modify: `internal/tui/app.go` — update references
- Modify: `internal/tui/views/agents.go` — k9s table styling, agent-centric columns
...
**Step 2: Rewrite the view with k9s-style columns**
...
```bash
git add -A
git commit -m "feat: k9s-style agents table with colored headers"
```
...
| 7 | Header | k9s-style info boxes + ASCII logo |
| 8 | Agents Table | k9s-style colored table, agent-centric columns |
